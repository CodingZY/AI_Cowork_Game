...
>(({ className, children, hideClose, ...props }, ref) => (
  <DialogPortal>
    ...
    <DialogPrimitive.Content
      ref={ref}
      className={cn(
        'fixed left-1/2 top-1/2 z-50 grid w-full max-w-lg -translate-x-1/2 -translate-y-1/2 gap-4 border border-line bg-surface p-5 shadow-xl rounded-xl',
    ...
    >
      {children}
      {!hideClose && (
        <DialogPrimitive.Close className="absolute right-4 top-4 rounded-md p-1 text-ink-3 opacity-80 transition hover:bg-surface-2 hover:text-ink focus:outline-none">
          <X className="size-4" />

...
```bash
# 0. 基础设施（Temporal / Redis / MySQL / Langfuse，docker 常驻，先 docker ps 确认）
docker ps

# Python 统一用 conda agent_env，从仓库根目录启动（设 PYTHONPATH 免 cd）
PY=D:/Anaconda3/envs/agent_env/python.exe

# 1. 后端 API :8000（--reload-dir 必加，否则 uvicorn 扫 frontend/node_modules 卡死）
PYTHONPATH=backend $PY -m uvicorn app.main:app --reload --reload-dir backend --host 127.0.0.1 --port 8000

# 2. Temporal Workflow Worker
PYTHONPATH=backend $PY -m temporal.worker

# 3. Arq Worker（仅 run_finalize，git 收尾落库；注意：指向 WorkerSettings 类，非模块）
PYTHONPATH=backend $PY -m arq app.queue.worker.WorkerSettings

# 4. 前端 :5173（Vite 冷启动约 26s）
